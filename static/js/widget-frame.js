    ...
    _list: [], _lastId: 1000000, _inited: false, init: function () {
        ...
        if (ge1('.js-popup_box', popup_el)) {
            addEvent(popup_el, 'click', function (e) {
                if (elInBody(e.target) && !hasClass(e.target, 'js-popup_box') && !gpeByClass(e.target, 'js-popup_box')) {
...
function showAlert(html, onClose) {
    return TPopups.show(html, [{label: l('WEB_CLOSE', 'Close'), enter: true, close: true}]);
}
...
function showConfirm(html, onConfirm, confirm_btn, onCancel, cancel_btn) {
    var popup_el = TPopups.show(html, [{
        ...
        label: confirm_btn || l('WEB_OK', 'OK'), onPress: function () {
...
function initWidgetFrame(options) {
    ...
    if (TWidget.options.auto_height || TWidget.options.auto_width) {
...
(function () {
    ...
    window.TPost = {
        init: function (postEl, options) {
            ...
            gec('.js-tgsticker_image', function () {
                ...
                RLottie.init(this, {playUntilEnd: this.hasAttribute('data-is-dice')});
            }, postEl);
            ...
            function recalcGrouped(max_w) {
                ...
                if (cnt == 2) {
                    ...
                    for (first_line = 1; first_line <= cnt - 1; first_line++) {
                        ...
                        tries.push([[first_line, second_line], [multiHeight(ratios_cropped.slice(0, first_line)), multiHeight(ratios_cropped.slice(first_line)),]]);
                    }
                    for (first_line = 1; first_line <= cnt - 2; first_line++) {
                        for (second_line = 1; second_line <= cnt - first_line - 1; second_line++) {
                            ...
                            if (first_line > 3 || second_line > (avg_ratio < 0.85 ? 4 : 3) || third_line > 3) {
                                continue;
                            }
                            tries.push([[first_line, second_line, third_line], [multiHeight(ratios_cropped.slice(0, first_line)), multiHeight(ratios_cropped.slice(first_line, first_line + second_line)), multiHeight(ratios_cropped.slice(first_line + second_line)),]]);
                        }
                    }
                    for (first_line = 1; first_line <= cnt - 3; first_line++) {
                        for (second_line = 1; second_line <= cnt - first_line - 2; second_line++) {
                            for (third_line = 1; third_line <= cnt - first_line - second_line - 1; third_line++) {
                                ...
                                tries.push([[first_line, second_line, third_line, fourth_line], [multiHeight(ratios_cropped.slice(0, first_line)), multiHeight(ratios_cropped.slice(first_line, first_line + second_line)), multiHeight(ratios_cropped.slice(first_line + second_line, first_line + second_line + third_line)), multiHeight(ratios_cropped.slice(first_line + second_line + third_line)),]]);
                            }
                        }
                    }
                    var opt_i = false;
    ...
    var TVoice = window.TVoice = {
        init: function (playerEl) {
            ...
            function seekStart(e) {
                if (player && player.duration !== Infinity && (!player.paused || player.currentTime > 0 && player.currentTime < player.duration)) {
                    e.preventDefault();
                    seeking = true;
                    disableClick = false;
                    player.pause();
            ...
            loadLib(TBaseUrl + 'js/ogvjs/ogv-support.js', function (success) {
                if (!success) return;
                if (isOGG && OGVCompat.hasWebAudio() && OGVCompat.supported('OGVPlayer')) {
    ...
    window.TWidgetPost = {
        init: function (options) {
            ...
            initWidgetFrame({
                auto_height: true, onVisible: function () {
                    gec('.js-widget_message', function () {
                        TPost.view(this);
                    });
                }
    ...
    window.TWidget = {initFrame: initWidgetFrame};
})();